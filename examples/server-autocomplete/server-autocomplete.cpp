...
// we expect client to format the prompt as needed
...
// REVIEW: is this the same for autocomplete?
enum task_type {
...
// for autocomplete we need more states
// REVIEW
enum slot_state
{
    IDLE,
    // PROCESSING,
    CONTEXT_PROCESSING, // Added for autocomplete: slot is catching up to the provided context
    SUGGESTION_PROCESSING, // Added for autocomplete: slot is generating suggestions up to the required suggestion length
    POST_SUGGESTION_PROCESSING, // Added for autocomplete: slot is generating tokens past the required suggestion length for the next suggestion
};

enum slot_command
{
    NONE,
    // REVIEW: we will have to change this for autocomplete
    // comment from this so we know we need to change this
    LOAD_PROMPT,
    RELEASE,
};
...
{   
    // REVIEW:
    // How does stream work for autocomplete?
    bool stream       = true;
    // REVIEW: 
    // this won't be used for autocomplete as we will be caching the prompts
    // The main difference  of the autocomplete is to cache prompts in clever ways
    // bool cache_prompt = false; // remember the prompt to avoid reprocessing all prompt

    uint32_t seed      = -1; // RNG seed
    // REVIEW: we will have to see how to deal with system prompts
    int32_t  n_keep    =  0; // number of tokens to keep from initial prompt
    int32_t  n_predict = -1; // new tokens to predict

    // std::vector<std::string> antiprompt;

    // this shouldn't be there for autocomplete
    // json input_prefix;
    // json input_suffix;
};
...
// REVIEW: we need a better common part for autocomplete
// comment from this so we know we need to change this
...
struct llama_client_slot
{
    ...
    // SS: does it mean the oldest slot?
...
struct llama_server_context
{
    ...
    // this is not used for autocomplete - client gives us formatted input
    // std::string name_user;      // this should be the antiprompt
    // std::string name_assistant;

    // slots / clients
    // REVIEW: how will we use slots in this?
    ...
    bool load_model(const gpt_params &params_)
    {
        ...
        /* if (!params.mmproj.empty()) {
            ...
            // clp_ctx = clip_model_load(params.mmproj.c_str(),.. 1);
            if(clp_ctx == nullptr) {
    ...
    void initialize() {
        ...
        // REVIEW: In Autocomplete, we will have to change this
        // it should be dynamic
        // const int32_t n_ctx_slot = n_ctx / params.n_parallel;
        const int32_t n_ctx_slot_ac = 0;

        // n_parallel will be the number of parallel contexts we will be keeping track of
        ...
        // empty system prompt
        // TODO: this should be updated for autocomplete
        system_prompt = "";
        system_tokens.clear();
    }
    ...
    llama_client_slot* get_slot(int id) {
        // get slot should be changed for autocomplete
        // it should return the last used slot
